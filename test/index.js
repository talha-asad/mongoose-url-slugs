...
TestObjSchema.plugin(urlSlugs('name', { max_slug_length : maxLength }));
...
describe('mongoose-url-slugs', function() {
  ...
  describe('max_length', function() {
    it('ensures slug length is less than max_length', function(done) {
      TestObj.create(
        { name : 'super duper long content that cannot possibly fit into a url in any meaningful manner' }
      , function(err, obj) {
    ...
    it('sequential slugs work with max_slug_length', function(done) {
      TestObj.create(
        { name : 'super duper long content that cannot possibly fit into a url' }
      , function(err, obj) {
          expect(obj.slug).length.to.be(maxLength)
          done();
        }
